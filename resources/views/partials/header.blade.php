<nav class="h-full flex flex-col px-4 py-6 gap-2">
    ...
    <div class="flex flex-col gap-2">
        <a href="{{ route('tasks.index') }}" title="Dashboard"
            class="flex items-center gap-3 rounded-md px-3 py-2.5 text-sm font-medium text-white bg-gray-950/50 hover:bg-gray-900 transition-colors">
        ...
        @can('manage_roles')
            <a href="{{ route('roles.index') }}" title="Gestión de roles"
                class="flex items-center gap-3 rounded-md px-3 py-2.5 text-sm font-medium text-gray-300 hover:bg-white/5 hover:text-white transition-colors">
                <svg class="w-5 h-5 shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
                    <path stroke-linecap="round" stroke-linejoin="round"
