                ...
                <tbody class="text-sm md:text-base">
                    @forelse ($users as $user)
                        <tr class="bg-[#1b1b18] hover:bg-[#252522] transition-colors rounded-lg">
                            ...
                            <td class="px-4 py-4 text-center">{{ $user->role }}</td>
                            <td class="px-4 py-4 text-center hidden md:table-cell">{{ $user->email }}</td>
                            <td class="px-4 py-4 rounded-r-lg">
                                <div class="flex items-center justify-center gap-4">

                                    {{-- Ver Info: Cargamos datos en selecteduser --}}
                                    <button @click="selectedUser = @js($user); showViewModalUser = true"
                                    ...
                                    <button class="text-indigo-400 hover:text-indigo-300 transition-colors" @click="selectedUser = { 
                                        id: {{ $user->id }}, 
                                        name: @js($user->name), 
                                        role: @js($user->role), 
                                        email: @js($user->email), 
                                    ...
                                    {{-- Eliminar --}}
                                    <form method="POST" action="{{ route('users.destroy', $user->id) }}" onsubmit="return confirm('¿Eliminar actividad?')">
                                        @csrf
                                        @method('DELETE')
                    ...
                    @empty
                        <tr>
                            <td colspan="6" class="py-10 text-center text-gray-500">No hay usuarios registrados</td>
                        </tr>
                    @endforelse
                </tbody>
            </table>
